	...
	return BaseController.extend("BenefitClaim.ZBenefitClaim.controller.ClaimForm", {
		...
		objectMatched: function (oEvent) {
			...
			if (oNameTile === "Admin" && oAdminHis === "F") {
				...
				if (oNameTile === "HistoryCoord" || oNameTile === "Coordinat") {
					...
					oFkeyss =
						"&$filter=REPORT eq 'Yes' and Rep_Status ne 'Error' and CATEGORY_CODE ne 'OC' and CATEGORY_CODE ne 'CPR' and CATEGORY_CODE ne 'CPC' and CATEGORY_CODE ne 'SDFR' and CATEGORY_CODE ne 'SDFC' and CATEGORY_CODE ne 'PAY_UP'";
				} else {
					oMkey = "/BenefietCAP/claim/Approval_Claim_Coordinator_Line(claim_Cordinator='" + oEmpId + "')/Set?$top=100000";
					oFkeyss =
						"&$filter=REPORT eq 'Yes' and Rep_Status ne 'Error' and CATEGORY_CODE eq 'OC' or CATEGORY_CODE eq 'CPR' or CATEGORY_CODE eq 'CPC' or CATEGORY_CODE eq 'SDFR' or CATEGORY_CODE eq 'SDFC' or CATEGORY_CODE eq 'PAY_UP'";
				}
		...
		onAddLineData: function (oEvent, model, Dmodel) {
			...
			if (model === "PUpload") {
				if (oMasterData.LINE_ITEM.length > 0) {
					if (oModel.CURRENCY !== oMasterData.LINE_ITEM[0].CURRENCY) {
						this._fnShowErrorMessage("Currency must be " + oMasterData.LINE_ITEM[0].CURRENCY);
						return;
					}
				}
				if (this.oViewData.getProperty("/PAYMENT") === "Vendor") {
					oModel.PAY_TO_BANK = "";
					oModel.ACC_NAME = "";
					oModel.ACC_NO = "";
		...
		_fnHistoryData: function (data) {
			if (data.value.length > 0) {
				var oModel = this._getSizeLimit(data.value),
					oValue = [];
				$.each(oModel.getData(), function (idx, obj) {
					if (obj.CLAIM_TYPE !== "PAY_UP") {
						oValue.push(obj);
					}
				});
				this.getView().setModel(new JSONModel(oValue), "ApprovalModel");
			} else {
